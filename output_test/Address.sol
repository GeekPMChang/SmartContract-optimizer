...
library Helpers {
 ...
 // ### PY_SOLOPT ### Found a rule violation of Loop Rule 6 - Loop fusion.
	...
	function bytesToBytes32(bytes memory b, uint offset) internal pure returns (bytes32) {
	  bytes32 out;
   // ### PY_SOLOPT ### Found a POSSIBLE rule violation of Loop Rule 3 Unrolling loop.
   // ### PY_SOLOPT ### Try to unroll the loop to improve the gas consumption.
   // ### PY_SOLOPT ### Found a rule violation of Loop Rule 5. Loop was replaced by do-while loop.
   ;
   do {
	  for (uint i = 0; i < 32; i++) {
       ;
