...
* Within the first year from deployment, the deployer of the RBST (Liquity AG's address) may transfer RBST only to valid 
* LockupContracts, and no other addresses (this is enforced in RBST.sol's transfer() function).
* 
* The above two restrictions ensure that until one year after system deployment, RBST tokens originating from Liquity AG cannot 
...
contract LockupContract {
    ...
    event LockupContractEmptied(uint _RBSTwithdrawal);
    ...
    function withdrawRBST() external {
        _requireCallerIsBeneficiary();
        _requireLockupDurationHasPassed();

        IRBST rbstCached = rbst;
        uint RBSTBalance = rbstCached.balanceOf(address(this));
        rbstCached.transfer(beneficiary, RBSTBalance);
        emit LockupContractEmptied(RBSTBalance);
    }
...
* LockupContracts. 
* 
* This registry is checked by RBST when the Liquity deployer attempts to transfer RBST tokens. During the first year 
* since system deployment, the Liquity deployer is only allowed to transfer RBST to valid LockupContracts that have been 
* deployed by and recorded in the LockupContractFactory. This ensures the deployer's RBST can't be traded or staked in the
...
* will be considered "valid" by RBST. This is a convenient way to verify that the target address is a genuine 
* LockupContract.
*/
...
contract LockupContractFactory is ILockupContractFactory, Ownable, CheckContract {
    ...
    function setRBSTAddress(address _rbstAddress) external override onlyOwner {
        checkContract(_rbstAddress);

        rbstAddress = _rbstAddress;
    ...
    function deployLockupContract(address _beneficiary, uint _unlockTime) external override {
        address rbstAddressCached = rbstAddress;
        _requireRBSTAddressIsSet(rbstAddressCached);
    ...
    // --- 'require'  functions ---
    function _requireRBSTAddressIsSet(address _rbstAddress) internal pure {
        require(_rbstAddress != address(0), "LCF: RBST Address is not set");
    }
}