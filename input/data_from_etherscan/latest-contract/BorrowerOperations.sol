...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    IRBSTStaking public rbstStaking;
    address public rbstStakingAddress;
    ...
    function setAddresses(
        ...
        onlyOwner
    {
        // This makes impossible to open a trove with zero withdrawn RUBC
        ...
        rbstStakingAddress = _rbstStakingAddress;
    ...
    function openTrove(uint _maxFeePercentage, uint _RUBCAmount, address _upperHint, address _lowerHint) external payable override {
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, rubc);
        ...
        vars.RUBCFee;
        vars.netDebt = _RUBCAmount;

        if (!isRecoveryMode) {
            vars.RUBCFee = _triggerBorrowingFee(contractsCache.troveManager, contractsCache.rubc, _RUBCAmount, _maxFeePercentage);
            vars.netDebt = vars.netDebt.add(vars.RUBCFee);
        }
        _requireAtLeastMinNetDebt(vars.netDebt);

        // ICR is based on the composite debt, i.e. the requested RUBC amount + RUBC borrowing fee + RUBC gas comp.
        ...
        // Move the ether to the Active Pool, and mint the RUBCAmount to the borrower
        _activePoolAddColl(contractsCache.activePool, msg.value);
        _withdrawRUBC(contractsCache.activePool, contractsCache.rubc, msg.sender, _RUBCAmount, vars.netDebt);
        // Move the RUBC gas compensation to the Gas Pool
        _withdrawRUBC(contractsCache.activePool, contractsCache.rubc, gasPoolAddress, RUBC_GAS_COMPENSATION, RUBC_GAS_COMPENSATION);

        emit TroveUpdated(msg.sender, vars.compositeDebt, msg.value, vars.stake, BorrowerOperation.openTrove);
        emit RUBCBorrowingFeePaid(msg.sender, vars.RUBCFee);
    }
    ...
    // Withdraw RUBC tokens from a trove: mint new RUBC tokens to the owner, and increase the trove's debt accordingly
    function withdrawRUBC(uint _maxFeePercentage, uint _RUBCAmount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _RUBCAmount, true, _upperHint, _lowerHint, _maxFeePercentage);
    }

    // Repay RUBC tokens to a Trove: Burn the repaid RUBC tokens, and reduce the trove's debt accordingly
    function repayRUBC(uint _RUBCAmount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _RUBCAmount, false, _upperHint, _lowerHint, 0);
    }

    function adjustTrove(uint _maxFeePercentage, uint _collWithdrawal, uint _RUBCChange, bool _isDebtIncrease, address _upperHint, address _lowerHint) external payable override {
        _adjustTrove(msg.sender, _collWithdrawal, _RUBCChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }
    ...
    */
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _RUBCChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, rubc);
        ...
        if (_isDebtIncrease) {
            _requireValidMaxFeePercentage(_maxFeePercentage, isRecoveryMode);
            _requireNonZeroDebtChange(_RUBCChange);
        }
        _requireSingularCollChange(_collWithdrawal);
        _requireNonZeroAdjustment(_collWithdrawal, _RUBCChange);
        _requireTroveisActive(contractsCache.troveManager, _borrower);

        // Confirm the operation is either a borrower adjusting their own trove, or a pure ETH transfer from the Stability Pool to a trove
        assert(msg.sender == _borrower || (msg.sender == stabilityPoolAddress && msg.value > 0 && _RUBCChange == 0));
        ...
        vars.netDebtChange = _RUBCChange;

        // If the adjustment incorporates a debt increase and system is in Normal Mode, then trigger a borrowing fee
        if (_isDebtIncrease && !isRecoveryMode) { 
            vars.RUBCFee = _triggerBorrowingFee(contractsCache.troveManager, contractsCache.rubc, _RUBCChange, _maxFeePercentage);
            vars.netDebtChange = vars.netDebtChange.add(vars.RUBCFee); // The raw debt change includes the fee
        }
        ...
        // When the adjustment is a debt repayment, check it's a valid amount and that the caller has enough RUBC
        if (!_isDebtIncrease && _RUBCChange > 0) {
            _requireAtLeastMinNetDebt(_getNetDebt(vars.debt).sub(vars.netDebtChange));
            _requireValidRUBCRepayment(vars.debt, vars.netDebtChange);
        ...
        // Use the unmodified _RUBCChange here, as we don't send the fee to the user
    ...
    function closeTrove() external override {
        ...
        // Burn the repaid RUBC from the user's balance and the gas compensation from the Gas Pool
        _repayRUBC(activePoolCached, rubcCached, msg.sender, debt.sub(RUBC_GAS_COMPENSATION));
        _repayRUBC(activePoolCached, rubcCached, gasPoolAddress, RUBC_GAS_COMPENSATION);
    ...
    function _triggerBorrowingFee(ITroveManager _troveManager, IRUBC _rubc, uint _RUBCAmount, uint _maxFeePercentage) internal returns (uint) {
        ...
        uint RUBCFee = _troveManager.getBorrowingFee(_RUBCAmount);

        _requireUserAcceptsFee(RUBCFee, _RUBCAmount, _maxFeePercentage);
        
        // Send fee to RBST staking contract
        //rbstStaking.increaseF_RUBC(RUBCFee);
        _rubc.mint(rbstStakingAddress, RUBCFee);
    ...
    {
        if (_isDebtIncrease) {
            _withdrawRUBC(_activePool, _rubc, _borrower, _RUBCChange, _netDebtChange);
        } else {
            _repayRUBC(_activePool, _rubc, _borrower, _RUBCChange);
        }
    ...
    // Issue the specified amount of RUBC to _account and increases the total active debt (_netDebtIncrease potentially includes a RUBCFee)
    function _withdrawRUBC(IActivePool _activePool, IRUBC _rubc, address _account, uint _RUBCAmount, uint _netDebtIncrease) internal {
        _activePool.increaseRUBCDebt(_netDebtIncrease);
        _rubc.mint(_account, _RUBCAmount);
    }

    // Burn the specified amount of RUBC from _account and decreases the total active debt
    function _repayRUBC(IActivePool _activePool, IRUBC _rubc, address _account, uint _RUBC) internal {
        _activePool.decreaseRUBCDebt(_RUBC);
        _rubc.burn(_account, _RUBC);
    }
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _RUBCChange) internal view {
        require(msg.value != 0 || _collWithdrawal != 0 || _RUBCChange != 0, "BorrowerOps: There must be either a collateral change or a debt change");
    }
    ...
    function _requireNonZeroDebtChange(uint _RUBCChange) internal pure {
        require(_RUBCChange > 0, "BorrowerOps: Debt increase requires non-zero debtChange");
    }
    ...
    function _requireValidRUBCRepayment(uint _currentDebt, uint _debtRepayment) internal pure {
        require(_debtRepayment <= _currentDebt.sub(RUBC_GAS_COMPENSATION), "BorrowerOps: Amount repaid must not be larger than the Trove's debt");
    }
    ...
    function _requireCallerIsStabilityPool() internal view {
     ...
     function _requireSufficientRUBCBalance(IRUBC _rubc, address _borrower, uint _debtRepayment) internal view {
        require(_rubc.balanceOf(_borrower) >= _debtRepayment, "BorrowerOps: Caller doesnt have enough RUBC to make repayment");
    }
