...
* 
*
*  --- Functionality added specific to the RBST ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending RBST directly to a Liquity
* core contract, when they should rather call the right function.
*
* 2) sendToRBSTStaking(): callable only by Liquity core contracts, which move RBST tokens from user -> RBSTStaking contract.
*
...
* -sendToRBSTStaking() reverts when the multisig is the sender, blocking the multisig from staking its RBST.
* 
* After one year has passed since deployment of the RBST, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract RBST is CheckContract, IRBST {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "RBST";
    string constant internal _SYMBOL = "RBST";
    ...
    address public immutable rbstStakingAddress;
        ...
        public 
    {
        ...
        uint _lpRewardsEntitlement = _1_MILLION.mul(10);  // Allocate 10 million for LP rewards
        lpRewardsEntitlement = _lpRewardsEntitlement;
        _mint(_lpRewardsAddress, _lpRewardsEntitlement);

        uint ukraineEntitlement = _1_MILLION.mul(20); // Allocate 20 million for Ukraine Relief
        _mint(_ukraineAddress, ukraineEntitlement);

        uint kekEntitlement = _1_MILLION.mul(7); // Allocate 7 million for the keks
        _mint(_kekAddress, kekEntitlement);

        uint financerEntitlement = _1_MILLION.mul(18); // Allocate 18 million for the financer
        _mint(_financerAddress, financerEntitlement);

        // Allocate the remainder to the RBST Multisig: (100 - 25 - 10 - 7 - 18) million = 20 million
        uint multisigEntitlement = _1_MILLION.mul(100)
            .sub(depositorsAndFrontEndsEntitlement)
            .sub(_lpRewardsEntitlement)
            .sub(ukraineEntitlement)
            .sub(kekEntitlement)
            .sub(financerEntitlement);
    ...
    function sendToRBSTStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsRBSTStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from staking RBST
        _transfer(_sender, rbstStakingAddress, _amount);
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'RBST: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'RBST: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "RBST: Cannot transfer tokens directly to the RBST token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != rbstStakingAddress,
            "RBST: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "RBST: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "RBST: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "RBST: caller must not be the multisig");
    }

    function _requireCallerIsRBSTStaking() internal view {
         require(msg.sender == rbstStakingAddress, "RBST: caller must be the RBSTStaking contract");
    }
